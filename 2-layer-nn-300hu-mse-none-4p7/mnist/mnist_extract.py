...
class MnistExtractor:
    ...
    def _extract_image_batch(self, list_of_indices):
        ...
        for i, index in enumerate(list_of_indices):
            self.fimage.seek(MNIST_OFFSETS["image"] + index*MNIST_DIMENSIONS["image"], os.SEEK_SET)
    ...
    def _extract_label_batch(self, list_of_indices):
        ...
        for i, index in enumerate(list_of_indices):
            self.flabel.seek(MNIST_OFFSETS["label"] + index, os.SEEK_SET)
            label_int = int.from_bytes(self.flabel.read(1), byteorder="big", signed=False)
            np_array[i, label_int] = 1
        LOG.debug(f"Extracting one-hot-encoded label batch ({list_of_indices})")
        return np_array
    ...
    def extract_batch(self, list_of_indices):
        ...
        """
        LOG.debug(f"Extracting a image/label batch ({list_of_indices})")
        return self._extract_image_batch(list_of_indices), self._extract_label_batch(list_of_indices)

    def close(self):
        """Closes file objects"""
        LOG.info(f"Closing file objects for {self.fimage.name} and {self.flabel.name}")
        self.fimage.close()
        self.flabel.close()
